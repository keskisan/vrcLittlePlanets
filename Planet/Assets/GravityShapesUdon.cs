...
public class GravityShapesUdon : UdonSharpBehaviour
{
    [SerializeField]
    CustomGravityUdon customGravity;

    [SerializeField]
    int objectType = 0; //0 sphere, 1 plane, 2 box

    [SerializeField]
    float gravity = 9.81f;

    [SerializeField, Min(0f)]
    float sInnerFalloffRadius = 1f, sInnerRadius = 5f;

    [SerializeField, Min(0f)]
    float sOuterRadius = 10f, sOuterFalloffRadius = 15f;

    float sInnerFalloffFactor, sOuterFalloffFactor;
    ...
    public Vector3 GetGravity(Vector3 position)
    {
        if (objectType == 0) //Sphere
        {
            ...
            if (distance > sOuterFalloffRadius || distance < sInnerFalloffRadius)
            {
                return Vector3.zero;
            }
            float g = gravity / distance;
            if (distance > sOuterRadius)
            {
                g *= 1f - (distance - sOuterRadius) * sOuterFalloffFactor;
            }
            else if (distance < sInnerRadius)
            {
                g *= 1f - (sInnerRadius - distance) * sInnerFalloffFactor;
            }
            return g * vector;
        }
    ...
    void Start()
    {
        if (objectType == 0) //Sphere
        {
            sInnerFalloffRadius = Mathf.Max(sInnerFalloffRadius, 0f);
            sInnerRadius = Mathf.Max(sInnerRadius, sInnerFalloffRadius);
            sOuterRadius = Mathf.Max(sOuterRadius, sInnerRadius);
            sOuterFalloffRadius = Mathf.Max(sOuterFalloffRadius, sOuterRadius);

            sInnerFalloffFactor = 1f / (sInnerRadius - sInnerFalloffRadius);
            sOuterFalloffFactor = 1f / (sOuterFalloffRadius - sOuterRadius);
        }
    ...
    void OnDrawGizmos()
    {
        if (objectType == 0) //Sphere
        {
            ...
            if (sInnerFalloffRadius > 0f && sInnerFalloffRadius < sInnerRadius)
            {
                Gizmos.color = Color.cyan;
                Gizmos.DrawWireSphere(p, sInnerFalloffRadius);
            }
            Gizmos.color = Color.yellow;
            if (sInnerRadius > 0f && sInnerRadius < sOuterRadius)
            {
                Gizmos.DrawWireSphere(p, sInnerRadius);
            }
            Gizmos.DrawWireSphere(p, sOuterRadius);
            if (sOuterFalloffRadius > sOuterRadius)
            {
                Gizmos.color = Color.cyan;
                Gizmos.DrawWireSphere(p, sOuterFalloffRadius);
            }
        }
        else if (objectType == 1) //Plane
        {
